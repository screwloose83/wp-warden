...
scan_site(){
 ...
 php "$WARDEN" "$SITE_ROOT" --verify-all --repair-original-auto --apply --fetch-official-checksums --noninteractive --quarantine-malware-auto --cleanup-malware-users-auto --cleanup-database-persistence-auto --quarantine-extra-auto --quarantine-extra-core-auto --exclude-pdf --max-size=1 --max-text-size=1 --quarantine="$QUARANTINE" 2>&1 | tee -a "$SITE_LOG"
