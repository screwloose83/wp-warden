...
const WP_WARDEN_VERSION = '0.1.57';
...
function audit_database_persistence(mysqli $db, string $prefix): void {
    ...
    foreach ($knownMeta as $metaKey => [$severity, $ruleId, $reason]) {
        ...
        $sql = "SELECT user_id, meta_key, LEFT(meta_value,512) AS meta_value FROM {$metaTable} WHERE meta_key='{$safe}' LIMIT 100";
        $res = @$db->query($sql);
        if (!$res) {
            continue;
        }
    ...
    // 3) Search option values for high-confidence persistence IOCs or embedded
    ...
    // 4) Cron is serialized data, so inspect its raw option for confirmed IOC
